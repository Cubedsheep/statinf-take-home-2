...
\noindent \fbox{\parbox{\linewidth}{Let $\epsilon_i$ be i.i.d., $\sigma^2 < \infty$, and let $Q$ be finite and non-singular. Consider the test
\begin{equation*}
	H_0: R\beta=r,
\end{equation*}
where $R$ is $(1\timesk)$ and $r$ is a a scalar, both known. Then
\begin{equation*}
	\frac{R\hat{\beta}-r}{\sqrt{s^2R(X^TX)^{-1}R^T}} \Rightarrow N(0,1)
\end{equation*}
}}
...
% R-code
\lstinputlisting{code1.R}
\lstinputlisting{code2.R}
\lstinputlisting{code3.R}
\lstinputlisting{code4.R}

%notebooks
\includepdf[pages=-]{Untitled1.pdf}
